...
def convert(cfg):
    print("convert user_id: ", user_id)
    cfg.checkpoint = cfg.checkpoint[:-3] + "-" + user_id + ".pt"
    ...
    synthesis_list_path = Path(
        utils.to_absolute_path(cfg.synthesis_list[:-5] + "_" + user_id + ".json"))  # ???인걸 보니 우리가 파이썬에서 돌릴때 지정해줘야함
    ...
    print("Load checkpoint from: {}:".format(
        cfg.checkpoint))  ### ???로 되어있는데 pretrained, 혹은 checkpoint까지 학습된 모델 있으면 그 모델의 위치로 지정
    checkpoint_path = utils.to_absolute_path(cfg.checkpoint)
    checkpoint = torch.load(checkpoint_path,
                            map_location=lambda storage, loc: storage)  # checkpoint에 지정된 weight들을 불러옵니다
    ...
    meter = pyloudnorm.Meter(
        cfg.preprocessing.sr)  # sr:16000으로 조정??  https://www.christiansteinmetz.com/projects-blog/pyloudnorm 소음 관련같습니다..

    for wav_path, speaker_id, out_filename in tqdm(
            synthesis_list):  # "english/test/S002_0379088085","V002","V002_0379088085"
